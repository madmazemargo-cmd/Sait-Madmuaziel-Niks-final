...
function CalendarPage({ session }: { session: Session }) {
  ...
  const gridEnd = view === 'week' ? masterAddDays(gridStart, 6) : masterMonthGridEnd(cursor);
  const gridLength = Math.round((masterDate(gridEnd).getTime() - masterDate(gridStart).getTime()) / 86400000) + 1;
