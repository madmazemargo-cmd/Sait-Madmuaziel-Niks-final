...
function MasterLayout({ session, children }: { session: Session; children: React.ReactNode }) {
  ...
  return <main className="master-shell"><div className="master-topbar"><Link className="brand" href="/master/calendar"><span className="brand-mark">N</span><span>МАДМУАЗЕЛЬ НИКС</span></Link><div className="master-user"><span>{session.user.login}</span><button type="button" onClick={logout} aria-label="Выйти"><LogOut size={15} /></button></div></div><div className="master-nav"><Link href="/master/calendar"><CalendarDays size={15} /> Календарь</Link><Link href="/master/catalog"><BookOpen size={15} /> Каталог</Link><Link href="/master/applications">Заявки</Link><Link href="/"><ArrowLeft size={14} /> На сайт</Link></div>{logoutError && <div className="master-error" role="alert">{logoutError}</div>}{children}</main>;
}
...
function CatalogEditor({ item, session, onSaved, onCancel }: { item: CatalogItem | null; session: Session; onSaved: () => void; onCancel: () => void }) {
  ...
  return <form className="master-editor" onSubmit={submit}>
...
function CatalogPage({ session }: { session: Session }) {
  ...
  return <MasterLayout session={session}>
    ...
    {editing !== undefined && <div className="master-overlay"><CatalogEditor item={editing} session={session} onSaved={() => { setEditing(undefined); load(); }} onCancel={() => setEditing(undefined)} /></div>}
  </MasterLayout>;
}
